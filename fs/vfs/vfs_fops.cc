...
bool vfs_file::map_page(uintptr_t off, mmu::hw_ptep<0> ptep, mmu::pt_element<0> pte, bool write, bool shared)
{
    ...
    if (vp->v_op->vop_get_page_addr) {
        ...
        vn_unlock(vp);
        assert(page_address);
        debugf("vfs_file::map_page of %s got page at %p for offset: %d, write: %d, shared: %d\n",
                fp->f_dentry->d_path, page_address, off, write, shared);
...
bool vfs_file::put_page(void *addr, uintptr_t off, mmu::hw_ptep<0> ptep)
{
    ...
    if (vp->v_op->vop_get_page_addr) {
        ...
        debugf("vfs_file::put_page of %s for address %p\n", fp->f_dentry->d_path, addr);
        mmu::clear_pte(ptep);
        return true;
    }
    else {
